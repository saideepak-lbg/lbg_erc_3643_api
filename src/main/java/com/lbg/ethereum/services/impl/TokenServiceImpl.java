...
public class TokenServiceImpl implements TokenService {
    ...
    public CreateOnChainIdResponse createOnChainId(OnChainIdCreationDto onChainIdCreationDto) {
        ...
        try {
            Credentials credentials = Credentials.create(environment.getProperty("lbgPrivateKey")); // This is your "signer"
            IdentityProxy identityProxy = IdentityProxy.deploy(
                    web3j,
                    credentials,
                    new DefaultGasProvider(),
                    identityImplementationAuthorityAddress,
                    environment.getProperty(onChainIdCreationDto.getUserAddress())          // same as process.env[body.userAddress]
            ).send();
    ...
    public ResponseEntity<RegisterIdentityReponseDto> registerIdentity(RegisterIdentityDto registerIdentityDto) {
        ...
        try {
            ...
            TransactionReceipt tx = registry.registerIdentity(
                    environment.getProperty(registerIdentityDto.getUserAddress()),
           ...
           response.setMessage("Failed to register identity: " + e.getMessage());
